...
namespace akf_lio
{
                             ...
                             int &N)
    {
        ...
        init_P(3, 3) = init_P(4, 4) = init_P(5, 5) = 0.000001;
        ...
        est_noise_.block<3, 3>(0, 0) = cov_gyr_scale_.asDiagonal();
        est_noise_.block<3, 3>(3, 0) = cov_acc_scale_.asDiagonal();
        est_noise_.block<3, 3>(6, 0) = cov_bias_gyr_.asDiagonal();
        est_noise_.block<3, 3>(9, 0) = cov_bias_acc_.asDiagonal();
    }
                                  ...
                                  PointCloudType &pcl_out)
    {
        ...
        if (last_dt_square_sum > 0)
        {
            ...
            auto fw_t = kf_state.last_fw_total;
            ...
            cur_noise.block<3, 3>(0, 0) = (fw_t.block(3, 0, 3, 3).inverse() * new_AQ.block<3, 3>(3, 3) *
                                           fw_t.block(3, 0, 3, 3).transpose().inverse());
            cur_noise.block<3, 3>(3, 0) = (fw_t.block(12, 3, 3, 3).inverse() * new_AQ.block<3, 3>(12, 12) *
                                           fw_t.block(12, 3, 3, 3).transpose().inverse());
            cur_noise.block<3, 3>(6, 0) = (fw_t.block(15, 6, 3, 3).inverse() * new_AQ.block<3, 3>(15, 15) *
                                           fw_t.block(15, 6, 3, 3).transpose().inverse());
            cur_noise.block<3, 3>(9, 0) = (fw_t.block(18, 9, 3, 3).inverse() * new_AQ.block<3, 3>(18, 18) *
                                           fw_t.block(18, 9, 3, 3).transpose().inverse());

            auto last_noise = est_noise_;
            est_noise_ = (1 - alpha) * cur_noise + alpha * last_noise;
        }
        // Q_.setZero();
        Q_.block<3, 3>(0, 0) = est_noise_.block<3, 3>(0, 0);
        Q_.block<3, 3>(3, 3) = est_noise_.block<3, 3>(3, 0);
        Q_.block<3, 3>(6, 6) = est_noise_.block<3, 3>(6, 0);
        Q_.block<3, 3>(9, 9) = est_noise_.block<3, 3>(9, 0);
