...
for i in range(1,4):
    for j in range(10):
        axs[j%5, j//5].plot(time,  a_out[:,i+j*3]-a_pre[:,i+j*3],'.-', label=lab_pre[i])
        # axs[j%5, j/5].plot(time, a_out[:,i+j*3],'.-', label=lab_out[i])
for j in range(10):
